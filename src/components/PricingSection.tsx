...
const pricingPlans: PricingPlan[] = [
  ...
  },
  {
    ...
    unit: "/ly",
    features: [
      "Đổi thiết kế theo mùa",
      "Đổi mẫu in theo ý muốn",
      "Miễn phí khuông vĩnh viễn",
      "Freeship toàn quốc",
      "Free phần mềm bán hàng",
      "Free thiết kế menu",
    ],
    highlight: false,
    dark: false,
        ...
        <div className="flex flex-col lg:flex-row lg:items-end gap-10 lg:gap-20 mb-20">
          ...
          <div className="lg:max-w-sm lg:pb-2">
            <p className="text-sm border-l-2 border-slate-300 pl-4 py-1 text-slate-500 font-medium leading-relaxed">
              Chi tiết đến từng đồng. Linh hoạt cho giai đoạn  thị trường đến mở rộng chuỗi quy mô toàn quốc.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {pricingPlans.map((plan, i) => {
            ...
            return (
              <div
                key={i}
                className={`relative rounded-2xl md:rounded-[2rem] p-8 lg:p-10 flex flex-col transition-all duration-300 ${isDark
              ...
              >
                {plan.badge && (
                  <div className="absolute -top-3 right-8 bg-red-500 text-white text-[10px] font-bold px-4 py-1.5 rounded-full uppercase tracking-wider">
                    {plan.badge}
                  </div>
                )}

                <div className="mb-8 border-b border-current/10 pb-8">
                  <h3 className={`text-xs font-bold tracking-widest uppercase mb-5 ${isDark ? "text-slate-500"
                  ...
                  <div className={`text-xs font-bold uppercase tracking-widest mt-4 ${isDark ? "text-[#f8eb96]"
                ...
                <ul className="flex-grow space-y-4 mb-10">
                  {plan.features.map((feature, j) => {
                    const isHot = feature.startsWith("Free");
                    return (
                    <li key={j} className="flex items-start gap-3">
                      <span className={`mt-0.5 text-xs ${isDark ? "text-[#f8eb96]"
                        : isOlive ? "text-[#f8eb96]"
                          : "text-[#6d8869]"
                        }`}>
                        {isHot ? "★" : "✓"}
                      </span>
                      <span className={`text-sm leading-relaxed ${isHot ? "font-bold" : "font-medium"} ${isDark ? "text-slate-300"
                        : isOlive ? "text-white/90"
                          : "text-slate-600"
                        }`}>
                        {feature}
                        {isHot && (
                          <span className="ml-2 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-red-500 text-white">Hot</span>
                        )}
                      </span>
                    </li>
                    );
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-full font-bold text-sm tracking-widest uppercase transition-all active:scale-95 ${isDark
