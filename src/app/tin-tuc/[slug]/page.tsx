 ...
 * SEO: generateMetadata, JSON-LD schema, canonical, OG tags
 */
...
// ─── JSON-LD Schema ───────────────────────────────────────────────────────────
function ArticleSchema({ post, siteUrl }: { post: { title: string; excerpt: string; thumbnail: string; publishedAt?: string; updatedAt?: string; author: { name: string; role?: string }; tags: readonly string[] | string[] }; siteUrl: string; slug: string }) {
  const schema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${siteUrl}/blog/${post.title}` },
  };
  return (
    <script
...
export default async function BlogPostPage(
  ...
  const post = dynPost
    ? {
        ...
        category: getCategoryBySlug(dynPost.categorySlug) ?? { name: dynPost.categorySlug, slug: dynPost.categorySlug },
      }
    : {
          ...
          <div className="absolute inset-0 flex items-end">
            <div className="max-w-[1400px] mx-auto px-6 md:px-12 pb-12 w-full">
              ...
              <nav aria-label="breadcrumb" className="flex items-center gap-2 text-sm mb-4">
                ...
                <span className="text-white/30">/</span>
                <Link href={`/tin-tuc/chuyen-muc/${post.category.slug}`} className="text-[#f8eb96] hover:text-white transition-colors">
                  {post.category.name}
                </Link>
              </nav>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <article className="lg:col-span-2">
              ...
              {post.tags.length > 0 && (
                <div className="mt-12 pt-8 border-t border-slate-200">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {post.tags.map((tag) => (
                      <span key={tag} className="px-4 py-1.5 bg-white border border-slate-200 rounded-full text-xs font-medium text-slate-500 hover:bg-[#1a1a1a] hover:text-[#f8eb96] hover:border-[#1a1a1a] transition-all cursor-default">
                        #{tag}
                      </span>
                    ))}
                  </div>
              ...
              <div className="mt-10 p-6 bg-white rounded-2xl shadow-sm flex items-start gap-4">
                ...
                <div>
                  ...
                  {post.author.role && <div className="text-xs text-[#6d8869] font-semibold uppercase tracking-wider mt-0.5">{post.author.role}</div>}
            ...
            <aside className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm">
                ...
                <Link href={`/tin-tuc/chuyen-muc/${post.category.slug}`}
              ...
              {related.length > 0 && (
                <div className="bg-white rounded-2xl p-5 shadow-sm">
                  ...
                  <div className="space-y-4">
                    {related.map((rp) => (
                      <Link key={rp.id} href={`/tin-tuc/${rp.slug}`} className="flex gap-3 group">
                        <div className="w-16 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-slate-100 relative">
                          <Image src={rp.thumbnail} alt={rp.title} fill className="object-cover group-hover:scale-105 transition-transform" />
                        </div>
                        <div>
